...
const getPagination = (page, size) => {
  ...
  const getPagingData = (data, page, limit) => {
    const { count: totalRoles, rows: roleInfo } = data;
    const currentPage = page ? +page : 0;
    const totalPages = Math.ceil(totalRoles / limit);

    return { totalRoles, roleInfo, totalPages, currentPage };
  };
...
exports.findAll = (req, res) => {
  ...
  Role.findAndCountAll({ 
    where: condition, offset, limit, order:[[sortColumn ? sortColumn : 'id', sort ? sort : 'ASC']], attributes: ['id', 'name', 'createdAt', 'updatedAt']
  })
    .then(data => {
      if (!data) {
